...
(103, 'Kabir Khanna', 'Python', 91, 96, 'Ms. Noir','2023-06-20', 'Mumbai'),
...
Select * , rank() over (order by score Desc) as ranking from Student_Performance;

#Q2: Show each student's score and the previous student’s score (based on score order).

Select name,score, lag(score) over (order by score) as Previous_Student_score from Student_Performance;

#Q3: Convert all student names to uppercase and extract the month name from join_date.

Select upper(name) as UpperCaseName,monthname(join_date) as Month_name from Student_Performance;

#Q4: Show each student's name and the next student’s attendance (ordered by attendance).

Select name, lead (attendance) over(order by attendance) as Next_Student_Attendence from Student_Performance;

#Q5: Assign students into 4 performance groups using NTILE().

SELECT name, course, score,NTILE(4) OVER (ORDER BY score DESC) AS Performance_Quarter from Student_Performance;

#Q6: For each course, assign a row number based on attendance (highest first).

SELECT course, attendance,ROW_NUMBER() OVER (PARTITION BY course ORDER BY attendance DESC) AS Row_no
...
Select name,join_date,datediff('2025-01-01',join_date) as days_since_joined from Student_Performance;

#Q8: Format join_date as “Month Year” (e.g., “June 2023”)

Select name, date_format(join_date,'%M %Y') as Month_Year from Student_Performance;
...
SELECT name, course, score, city_display
FROM City_Transformation;

#10: For each course, find the highest score using FIRST_VALUE().
select name,Course,score ,first_value(score) over (partition by course
order by score desc) as Course_Highest_Score
from Student_Performance;
